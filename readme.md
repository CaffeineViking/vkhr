...
In the above plot to the left we see how the rasterizer and raymarcher fare at different distances, and how much time each of the rendering passes take. For the near cases (e.g. a character close-up) the raymarched solution is around twice as fast, but the fidelity isn't very good, as strands are lumped together because of the volume approximations. On the other hand, the rasterized solution produces high-quality output as each strand is individually distinguishable. However for far away to medium distances, these small details are not noticable, and the rasterized and raymarched solution are indistinguishable. The raymarcher on the other hand is now 5x faster in these distances! It has better scaling with distance for far away shots.
