...
float filtered_shadows(sampler2D shadow_map,
    ...
    float kernel_range = (pcf_kernel_width - 1.00f) / 2.0f;
    float kernel_total = pcf_kernel_width*pcf_kernel_width;

    for (float y = -kernel_range; y <= +kernel_range; y += 1.0f)
    for (float x = -kernel_range; x <= +kernel_range; x += 1.0f) {
        float shadow_depth = tex2Dproj(shadow_map, light_space_vertex,
                                       vec2(x, y) / shadow_map_size).r;
        if (shadow_depth > light_depth - shadow_map_bias)
            visibility -= 1.0f / kernel_total;
    }
